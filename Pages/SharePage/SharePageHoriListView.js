  ...
  _renderRow(rowData) {
    return (
      <SharePageHoriListViewItem
        onClickImage = {() => this.props.fetchModal(true, <ImageViewerPage imageURL={rowData.uri}/>)}
        shareImageURI={rowData.uri}
        isUserLike={rowData.isUserLike}
        onLikeBtnClicked={(isLiked) => console.log('art #' + rowData.pk + ': ' + isLiked)}/>
    );
  }
    ...
    return (
        <View style={styles.container}>
          <View style={styles.shareKeywordContainer}>
            <Text style={styles.headingText}>{this.props.keyword}</Text>
          </View>
          <UpperLinearGradient/>
